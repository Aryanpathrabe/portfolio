    ...
    return (
      <div style={{ minHeight: "100vh", background: "#0d0d0d", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ background: "#161616", border: "1px solid #2a2a2a", padding: "2.5rem", width: "100%", maxWidth: "360px", borderRadius: "8px" }}>
          <h1 style={{ fontFamily: "'Jost', sans-serif", fontWeight: 800, color: "#fafafa", fontSize: "1.5rem", marginBottom: "0.5rem" }}><span style={{ color: accent }}>&lt;</span>Admin<span style={{ color: accent }}> /&gt;</span></h1>
          <p style={{ color: "#666", fontSize: "0.85rem", marginBottom: "1.5rem", fontFamily: "'DM Sans', sans-serif" }}>Portfolio control panel</p>
          <label style={labelStyle}>Password</label>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && (password === ADMIN_PASSWORD ? setAuthed(true) : flash("Wrong password!"))} style={{ ...inputStyle, marginBottom: "1rem" }} placeholder="Enter password" />
          {msg && <p style={{ color: accent, fontSize: "0.8rem", marginBottom: "0.75rem", fontFamily: "'DM Mono', monospace" }}>{msg}</p>}
          <button onClick={() => password === ADMIN_PASSWORD ? setAuthed(true) : flash("Wrong password!")} style={{ ...btnStyle, background: accent, color: "#fff", width: "100%" }}>Login</button>
        </div>
      </div>
    );
  }
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0d", color: "#fafafa", fontFamily: "'DM Sans', sans-serif" }}>
      <header style={{ background: "#111", borderBottom: "1px solid #2a2a2a", padding: "1rem 2rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ fontFamily: "'Jost', sans-serif", fontWeight: 800, fontSize: "1.1rem" }}><span style={{ color: accent }}>&lt;</span>Admin Panel<span style={{ color: accent }}> /&gt;</span></h1>
        <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
          {msg && <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.7rem", color: "#2dd4ac" }}>{msg}</span>}
          <button onClick={() => window.location.href = "/"} style={{ ...btnStyle, background: "transparent", color: "#666", border: "1px solid #2a2a2a" }}>← Portfolio</button>
          <button onClick={() => setAuthed(false)} style={{ ...btnStyle, background: "#1a1a1a", color: "#fafafa", border: "1px solid #2a2a2a" }}>Logout</button>
        </div>
      </header>
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "2rem" }}>
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "2rem" }}>
          {(["projects", "skills"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} style={{ ...btnStyle, background: tab === t ? accent : "#1a1a1a", color: tab === t ? "#fff" : "#aaa", border: "1px solid #2a2a2a" }}>{t}</button>
          ))}
        </div>
        {tab === "projects" && (
            ...
            <div style={{ background: "#111", border: "1px solid #2a2a2a", borderRadius: "8px", padding: "1.5rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
                {([{ label: "Number (e.g. 01)", key: "num" }, { label: "Title", key: "title" }, { label: "Role (e.g. Core · 2024)", key: "role" }, { label: "Image URL", key: "img" }, { label: "Project Link", key: "link" }, { label: "Tags (comma separated)", key: "tags" }] as { label: string; key: keyof Project }[]).map(({ label, key }) => (
                  <div key={key}><label style={labelStyle}>{label}</label><input style={inputStyle} value={Array.isArray(projectForm[key]) ? (projectForm[key] as string[]).join(", ") : projectForm[key] as string} onChange={(e) => setProjectForm({ ...projectForm, [key]: e.target.value })} placeholder={label} /></div>
                ))}
                <div><label style={labelStyle}>Category</label><select value={projectForm.category} onChange={(e) => setProjectForm({ ...projectForm, category: e.target.value })} style={{ ...inputStyle }}><option value="Core">Core</option><option value="AI Projects">AI Projects</option></select></div>
                <div><label style={labelStyle}>Description</label><textarea rows={3} style={{ ...inputStyle, resize: "vertical" }} value={projectForm.description} onChange={(e) => setProjectForm({ ...projectForm, description: e.target.value })} placeholder="Project description" /></div>
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <button onClick={saveProject} style={{ ...btnStyle, background: accent, color: "#fff", flex: 1 }}>{editingProject ? "Update" : "Add"} Project</button>
                  {editingProject && <button onClick={() => { setProjectForm(emptyProject); setEditingProject(null); }} style={{ ...btnStyle, background: "#1a1a1a", color: "#aaa", border: "1px solid #2a2a2a" }}>Cancel</button>}
                </div>
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              ...
              {projects.map((p) => (
                <div key={p.id} style={{ background: "#111", border: "1px solid #2a2a2a", borderRadius: "8px", padding: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div><p style={{ fontFamily: "'Jost', sans-serif", fontWeight: 700, fontSize: "0.95rem" }}>{p.num} — {p.title}</p><p style={{ color: "#666", fontSize: "0.8rem", fontFamily: "'DM Mono', monospace" }}>{p.category} · {p.role}</p></div>
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <button onClick={() => { setProjectForm({ ...p, tags: p.tags || [] }); setEditingProject(p.id!); }} style={{ ...btnStyle, background: "#1a1a1a", color: "#aaa", border: "1px solid #2a2a2a", fontSize: "0.72rem" }}>Edit</button>
                    <button onClick={() => deleteProject(p.id!)} style={{ ...btnStyle, background: "#2a0a0a", color: accent, border: `1px solid ${accent}`, fontSize: "0.72rem" }}>Delete</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab === "skills" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: "2rem", alignItems: "start" }}>
            <div style={{ background: "#111", border: "1px solid #2a2a2a", borderRadius: "8px", padding: "1.5rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
                <div><label style={labelStyle}>Skill Name</label><input style={inputStyle} value={skillForm.name} onChange={(e) => setSkillForm({ ...skillForm, name: e.target.value })} placeholder="e.g. React / Next.js" /></div>
                <div><label style={labelStyle}>Level: {skillForm.level}%</label><input type="range" min={0} max={100} value={skillForm.level} onChange={(e) => setSkillForm({ ...skillForm, level: Number(e.target.value) })} style={{ width: "100%", accentColor: accent }} /></div>
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <button onClick={saveSkill} style={{ ...btnStyle, background: accent, color: "#fff", flex: 1 }}>{editingSkill ? "Update" : "Add"} Skill</button>
                  {editingSkill && <button onClick={() => { setSkillForm(emptySkill); setEditingSkill(null); }} style={{ ...btnStyle, background: "#1a1a1a", color: "#aaa", border: "1px solid #2a2a2a" }}>Cancel</button>}
                </div>
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              ...
              {skills.map((s) => (
                <div key={s.id} style={{ background: "#111", border: "1px solid #2a2a2a", borderRadius: "8px", padding: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ flex: 1, marginRight: "1rem" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.4rem" }}><span style={{ fontFamily: "'Jost', sans-serif", fontWeight: 600, fontSize: "0.9rem" }}>{s.name}</span><span style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.7rem", color: accent }}>{s.level}%</span></div>
                    <div style={{ background: "#2a2a2a", height: "2px", borderRadius: "1px" }}><div style={{ background: accent, height: "2px", width: `${s.level}%`, borderRadius: "1px" }} /></div>
                  </div>
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <button onClick={() => { setSkillForm(s); setEditingSkill(s.id!); }} style={{ ...btnStyle, background: "#1a1a1a", color: "#aaa", border: "1px solid #2a2a2a", fontSize: "0.72rem" }}>Edit</button>
                    <button onClick={() => deleteSkill(s.id!)} style={{ ...btnStyle, background: "#2a0a0a", color: accent, border: `1px solid ${accent}`, fontSize: "0.72rem" }}>Delete</button>
                  </div>
                </div>
              ))}
            </div>
